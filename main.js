  ...
  setInterval(function(){
              ...
              connection.query('SELECT modo FROM controlador WHERE nombre = "motores"', function (error, results, fields) {
                if (error){
                  ...
                  if (modo !== String(results[0].modo)) { // La variable booleana es  para controlar el envío de los datos, se envían solo cuando hayan cambiado
                    ...
                    //De la siguietne forma, cuando ocurra un cambio de estado, deberán apagarse todos los motores
                    // Forzamos a que los motores tomen el estado actual en la base de datos
                    //if (modo === '1') {
                      m1_estado = false;
                      m2_estado = false;
                      m3_estado = false;
            ...
            if (!calibracion_oxd_vijia) {
              setTimeout(function() {
                ...
                console.log("Envía CAL", calibracion_oxd);
            ...
            if (!ox_minimo) {
              setTimeout(function() {
                ...
                console.log("Envía oxmin", ox_min);
                ox_minimo = true;
              }, 2000);
            }
            if (!ox_maximo) {
              setTimeout(function() {
                ...
                console.log("Envía oxmax", ox_max);
                ox_maximo = true;
              }, 3000);
            }
            if (!m1maxamperaje) {
...
var port_serie = new SerialPort("COM3", {
  baudRate: 115200
});
